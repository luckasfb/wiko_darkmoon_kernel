...
int ft5x06_get_vendor_version(tinno_ts_data *ts, uint8_t *pfw_vendor, uint8_t *pfw_version)
{
	...
	*pfw_version = ft5x06_get_fw_version(ts);
	...
	CTP_DBG("[CTP-FT5X06] fw_vendor=0x%X, fw_version=0x%X\n", *pfw_vendor, *pfw_version);
	return 0;
}
...
void ft5x06_tp_upgrade(const char * ftbin_buf, int buf_len)
{
	int vl_tp_ver = 0x00;
	...
	CTP_DBG("entry ft5x06_tp_upgrade \n");
	...
	if(vl_bin_ver > vl_tp_ver)
	{
		...
		for( i = 0; i < 3; i++ )
		{
			rc = ft5x06_get_tp_id(g_pts, &tp_id);
			CTP_DBG("TPID=0x%X!", tp_id);
			if(rc)
			{
				CTP_DBG("Get tp ID error(%d)", rc);
