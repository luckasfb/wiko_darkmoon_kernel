...
u32 pinSet[2][8] = {
    ...
    //BEGIN <camera> <DATE20130802> <power on> zhangxiaofei
    else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX111_MIPI_RAW, currSensorName)))
    {
		PK_DBG("kdCISModulePowerOn get in---  IMX111 \n");
        //enable active sensor
        //RST pin
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
            if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
    ...
    else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8850_MIPI_RAW, currSensorName)))
    {
		PK_DBG("kdCISModulePowerOn get in---  OV8850 \n");
        //enable active sensor
        //RST pin
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
            if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
    ...
    else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5648_MIPI_RAW, currSensorName)))
    {
        ...
        //LINE <> <20130911> <correct the DVDD name, voltage> panzaoyan
        if(TRUE != hwPowerOn(MT6323_POWER_LDO_VGP3, VOL_1500,mode_name)) // if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, MT6323_POWER_LDO_VGP3,mode_name))
        {
             PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
             //return -EIO;
             //goto _kdCISModulePowerOn_exit_;
    ...
    //END <camera> <DATE20130802> <power on> zhangxiaofei
    else
    {
		...
		//BEGIN <camera> <DATE20130802> <power off> zhangxiaofei
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX111_MIPI_RAW,currSensorName)))
		{
		   PK_DBG("kdCISModulePower--off get in---IMX111 \n");
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
		...
		{
		   PK_DBG("kdCISModulePower--off get in---OV8850 \n");
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5648_MIPI_RAW,currSensorName)))
		{
		   PK_DBG("kdCISModulePower--off get in---OV5648 \n");
            ...
            //}
              ...
              //LINE <> <20130911> <correct the DVDD name, voltage> panzaoyan
               if(TRUE != hwPowerDown(MT6323_POWER_LDO_VGP3, mode_name)) { //if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name)) {
               PK_DBG("[CAMERA SENSOR] Fail to OFF digital power\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }
		...
		//END <camera> <DATE20130802> <power off> zhangxiaofei
		else
	    {
