...
//Begin: Jovan added : just use to modify the difference of S8513  between black and white  phone
static int count_read_ps=0;    //jovan added 
static int ap3220_read_flag=0;    //jovan added 
static int ap3220_crosstalk_value=0;
static int black_shift_high=0;
static int black_shift_low=0;
//#define	ap3220_udelay(n)	waiton_timer2(((n)*TICKS_PER_MS)/1000)
static void ap3220_reset_all_vaule(void)
{
 count_read_ps=0;    //jovan added 
ap3220_read_flag=0;    //jovan added 
ap3220_crosstalk_value=0;
black_shift_high=0;
black_shift_low=0;
}

static int ap3220_modify_black_white(void)
{

	long res;
	u8 databuf[2];
	long int current_value;
	long count_average=0;

	//APS_LOG("Enter ap3220_modify_black_white jovan\n");
	while(count_read_ps!=4)
	{
	if(count_read_ps==0)
		{	
			res = ap3220_i2c_write_reg(0x00,0x02);    //need 12.5ms 
			mdelay(60);         		//wait for getting the PS_data as a condition
			//APS_LOG("35ms later: count_read_ps = %x\n",count_read_ps);
		}
	if(count_read_ps!=0)
		{
			mdelay(35);			//wait for getting the crosstalk_value of white after modify!
			//APS_LOG("get the crosstalk_value of white after modify: count_read_ps = %x\n",count_read_ps);
		}
	//res = ap3220_i2c_write_reg(0x00,0x03);
//	APS_FUN(f);
	res = ap3220_i2c_read_reg(AP3220_REG_SYS_PS_DATA_LOW,&databuf[0]);
	res = ap3220_i2c_read_reg(AP3220_REG_SYS_PS_DATA_HIGH,&databuf[1]);
	if(res < 0)
	{
		...
		//APS_LOG("AP3220_REG_PS_DATA value value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
	current_value = ((databuf[1] & 0x3f) << 4 |(databuf[0] & 0x0f));

		//APS_LOG("ap3220_read_flag modify  ap3220_read_flag=%d  jovan\n",ap3220_read_flag);	
		APS_LOG("ap3220_read_flag modify  current_value=%x,ap3220_crosstalk_value=%x jovan\n",current_value,ap3220_crosstalk_value);
	count_read_ps++;
	if(ap3220_read_flag==0)									//modify the black and white phone!
	{
	//	res = ap3220_i2c_write_reg(0x20,0x81);
	//	res = ap3220_i2c_write_reg(0x21,0x20);
	//	mdelay(50);
		if(current_value>0x100)		//white   
		{

			ap3220_read_flag=1;	
			black_shift_high=0;
			black_shift_low=0;
				//APS_LOG("ap3220_read_flag modify count_read_ps=%x,black_shift_high=%x,black_shift_low=%x jovan\n",count_read_ps,black_shift_high,black_shift_low);
		}
		else												//black
		{
			ap3220_read_flag=1;	
			black_shift_high=0x104;
			black_shift_low=0xA5;
				//APS_LOG(" ap3220_read_flag modify count_read_ps=%x,black_shift_high=%x,black_shift_low=%x jovan\n",count_read_ps,black_shift_high,black_shift_low);
		}
			//APS_LOG("exit ap3220_read_flag modify  ap3220_read_flag=%d  jovan\n",ap3220_read_flag);
	}
	else													//for adding the classtalk 
	{
			
			ap3220_crosstalk_value+=current_value;
			if(count_read_ps!=2)
			{
				for(count_average=0;count_average<5;count_average++)
				{

					ap3220_crosstalk_value/=2;
					if (count_average<4)
					{
						ap3220_crosstalk_value+=current_value;
					}
...
static int ap3220_threshold_high_low_reset(int th_high,int th_low)
{
	
	u16 value_high,value_low;
	int res = 0;

	value_high=th_high+ap3220_crosstalk_value+black_shift_high;
	value_low=th_low+ap3220_crosstalk_value+black_shift_low;

	if(value_high>0x3a0)			//It is used to make sure the phone can work normally if  the ps-window has been covered with something when the initinal of P-sensor is going. 
		{
			value_high=0x265;
			value_low=0x240;		
			ap3220_crosstalk_value=0;
			black_shift_high=0x149;
			black_shift_low=0x1BB;
			APS_LOG("The phone is covered with something when the initinal of P-sensor is going. And the value of threshold_high_low has be reset:ap3220_crosstalk_value=%x,black_shift_high=%x,black_shift_low=%x,th_high=%x,th_low=%x,value_high=%x,value_low=%x! @jovan\n",ap3220_crosstalk_value,black_shift_high,black_shift_low,th_high,th_low,value_high,value_low);
		}
	...
	APS_LOG("The value of threshold_high_low has be reset:th_high=%x,th_low=%x,value_high=%x,value_low=%x! @jovan\n",th_high,th_low,value_high,value_low);
	if(res < 0)
		{
			APS_ERR("i2c_master_send function err\n");
			return res;
...
//ALS PERSIST	
	...
	APS_LOG("Enter ap3220_init_client jovan\n");
...
//PS High Threshold H
	...
	res=ap3220_modify_black_white();     //get the average value.
	if(res < 0)
	{
		APS_ERR("i2c_master_send function err\n");
		goto EXIT_ERR;
	}
	res=ap3220_threshold_high_low_reset(atomic_read(&obj->ps_thd_val_high),atomic_read(&obj->ps_thd_val_low));
	if(res < 0)
	{
		APS_ERR("i2c_master_send function err\n");
		goto EXIT_ERR;
...
static int ap3220_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//Begin: added by jovan @20131014

	//recover_threshold_vale((obj->hw->ps_threshold_high)+ap3220_crosstalk_value+black_shift_high,(obj->hw->ps_threshold_low)+ap3220_crosstalk_value+black_shift_low);

	atomic_set(&obj->ps_thd_val_high, (obj->hw->ps_threshold_high)+ap3220_crosstalk_value+black_shift_high);
	atomic_set(&obj->ps_thd_val_low,  (obj->hw->ps_threshold_low)+ap3220_crosstalk_value+black_shift_low);
	//atomic_set(&obj->ps_thd_val_high,  (obj->hw->ps_threshold_high)+ap3220_crosstalk_value+black_shift_high);
	//atomic_set(&obj->ps_thd_val_low,  (obj->hw->ps_threshold_low)+ap3220_crosstalk_value+black_shift_low);


	ap3220_reset_all_vaule();
	
	//end: added by jovan @20131014
